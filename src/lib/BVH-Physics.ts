...
export function BVHPhysicsUpdate(delta: number, query: PhysicsQuery) {
	for (const { position, velocity, grounded } of query) {
		...
		// TODO-DefinitelyMaybe: player is a mesh
		const capsuleInfo = player.capsuleInfo;
		tempBox.makeEmpty();
		// TODO-DefinitelyMaybe: collider is the world/ground
		...
		const ObjectIsOnGround = deltaVector.y > Math.abs(delta * velocity.y * 0.25);
